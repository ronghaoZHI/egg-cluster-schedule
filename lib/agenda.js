...
module.exports = (agent) => {
  ...
  const addAgendaJob = async (ctx, opt) => {
    const { agent, name, corn, interval, immediate } = opt;
    ...
    if(corn) {
      ...
      await agenda.every(corn, name, {}, { skipImmediate: !immediate });
    } else if(interval) {
    ...
    start() {
      ;(async () => {
        ...
        if(schedule.type === 'cluster') {
          ...
          if(schedule.corn) {
            await addAgendaJob(this, { agent, name: key, corn: schedule.corn, immediate, })
